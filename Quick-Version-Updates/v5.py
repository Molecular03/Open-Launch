...
def euler_cromer():
    ...
    z, v, t = 1, 0.0, 0.1 #Initial altitude (m), Initial velocity (m/s), Initial time
    dt = 0.1 #0.1
    total_mass = variables[9] + variables[10]
    variables.append(total_mass)
    z_values, t_values = [], []
    while t < 1000: #t0 --> t max:
        variables[16] -= variables[15]*dt #Mass update from fuel use
        variables[1] = -(variables[0]*variables[2])/((z+6371000)**2) #Gravitational acceleration update as function of altitude
        variables[13], variables[12], variables[5] = density(variables[12],z) #Rho, temp, pressure update
        variables[7] = drag_coefficient(v, variables[12], variables[7]) #Drag coefficient
        variables[6] /= variables[16] #Thrust
        variables[15] = variables[6]/(-variables[1]*variables[11]) #mass flow rate update
        Fdrag = (variables[13]*(v**2)*variables[7]*variables[14])/2
        if v < 0:
            Fdrag *= -1
        v += (variables[6] - Fdrag - variables[1])*dt
        z += v*dt
        t += dt
        z_values.append(z)
        t_values.append(t)
        print(variables[16], variables[10], variables[15]*dt)
        if z <= 0 and t != 0: #Crash Condition
            print('Crashed!') #Change to a Tkinter output 
            break
        if variables[16] <= variables[10]: #If current mass is less than/equal to dry mass, no more propellant is available
